(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ks3_upload"] = factory(require("vue"));
	else
		root["ks3_upload"] = factory(root["Vue"]);
...
"use strict";
/* WEBPACK VAR INJECTION */(function(plupload) {/* harmony import */ var core_js_modules_es6_regexp_constructor__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__("3b2b");
...
(function KS3JsSDK(win) {
  var ks3FileUploader = function ks3FileUploader(ks3PostOptions, pluploadOptions) {
    this.defaultKS3Options = {
      KSSAccessKeyId: "",
      policy: "",
      //请求中用于描述获准行为的安全策略。没有安全策略的请求被认为是匿名请求，只能访问公共可写空间。
      signature: "",
      //根据Access Key Secret和policy计算的签名信息，KS3验证该签名信息从而验证该Post请求的合法性。
      bucket_name: "",
      //上传的空间名
      key: "",
      //被上传键值的名称。如果用户想要使用文件名作为键值，可以使用${filename} 变量。例如：如果用户想要上传文件local.jpg，需要指明specify /user/betty/${filename}，那么键值就会为/user/betty/local.jpg。
      acl: "private",
      //上传文件访问权限,有效值: private | public-read
      uploadDomain: "",
      //上传域名,http://destination-bucket.kss.ksyun.com 或者 http://kssws.ks-cdn.com/destination-bucket
      autoStart: false,
      //是否在文件添加完毕后自动上传
      onInitCallBack: function onInitCallBack() {},
      //上传初始化时调用的回调函数
      onErrorCallBack: function onErrorCallBack() {},
      //发生错误时调用的回调函数
      onFilesAddedCallBack: function onFilesAddedCallBack() {},
      //文件添加到浏览器时调用的回调函数
      onBeforeUploadCallBack: function onBeforeUploadCallBack() {},
      //文件上传之前时调用的回调函数
      onStartUploadFileCallBack: function onStartUploadFileCallBack() {},
      //文件开始上传时调用的回调函数
      onUploadProgressCallBack: function onUploadProgressCallBack() {},
      //上传进度时调用的回调函数
      onFileUploadedCallBack: function onFileUploadedCallBack() {},
      //文件上传完成时调用的回调函数
      onUploadCompleteCallBack: function onUploadCompleteCallBack() {} //所有上传完成时调用的回调函数
  ...
  ks3FileUploader.prototype.onInit = function (uploader, obj) {
    this.defaultKS3Options.onInitCallBack && this.defaultKS3Options.onInitCallBack.apply(this, [uploader, obj]);
  };

  ks3FileUploader.prototype.onUploadError = function (uploader, obj) {
    this.defaultKS3Options.onErrorCallBack && this.defaultKS3Options.onErrorCallBack.apply(this, [uploader, obj]);
  };

  ks3FileUploader.prototype.onFilesAdded = function (uploader, obj) {
    if (this.defaultKS3Options.autoStart) this.uploader.start();
    this.defaultKS3Options.onFilesAddedCallBack && this.defaultKS3Options.onFilesAddedCallBack.apply(this, [uploader, obj]);
  };

  ks3FileUploader.prototype.onBeforeUpload = function (uploader, obj) {
    this.defaultKS3Options.onBeforeUploadCallBack && this.defaultKS3Options.onBeforeUploadCallBack.apply(this, [uploader, obj]);
  };

  ks3FileUploader.prototype.onStartUploadFile = function (uploader, obj) {
    this.defaultKS3Options.onStartUploadFileCallBack && this.defaultKS3Options.onStartUploadFileCallBack.apply(this, [uploader, obj]);
  };

  ks3FileUploader.prototype.onUploadProgress = function (uploader, obj) {
    this.defaultKS3Options.onUploadProgressCallBack && this.defaultKS3Options.onUploadProgressCallBack.apply(this, [uploader, obj]);
  };

  ks3FileUploader.prototype.onFileUploaded = function (uploader, obj, resObj) {
    this.defaultKS3Options.onFileUploadedCallBack && this.defaultKS3Options.onFileUploadedCallBack.apply(this, [uploader, obj, resObj]);
  };

  ks3FileUploader.prototype.onUploadComplete = function (uploader, obj) {
    this.defaultKS3Options.onUploadCompleteCallBack && this.defaultKS3Options.onUploadCompleteCallBack.apply(this, [uploader, obj]);
  };
...
Ks3.binb2b64 = function (binarray) {
  ...
  for (var i = 0; i < binarray.length * 4; i += 3) {
    ...
    for (var j = 0; j < 4; j++) {
      if (i * 8 + j * 6 > binarray.length * 32) str += Ks3.b64pad;else str += tab.charAt(triplet >> 6 * (3 - j) & 0x3F);
    }
  }
...
Ks3.core_sha1 = function (x, len) {
  ...
  for (var i = 0; i < x.length; i += 16) {
    ...
    for (var j = 0; j < 80; j++) {
      if (j < 16) w[j] = x[i + j];else w[j] = Ks3.rol(w[j - 3] ^ w[j - 8] ^ w[j - 14] ^ w[j - 16], 1);
      var t = Ks3.safe_add(Ks3.safe_add(Ks3.rol(a, 5), Ks3.sha1_ft(j, b, c, d)), Ks3.safe_add(Ks3.safe_add(e, w[j]), Ks3.sha1_kt(j)));
      e = d;
      d = c;
      c = Ks3.rol(b, 30);
      b = a;
...
Ks3.generateToken = function (sk, bucket, resource, http_verb, content_type, headers, time_stamp) {
  ...
  var signature = Ks3.b64_hmac_sha1(sk, string2Sign); //console.log('signature:' + signature);
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4) {
      if (xhr.status >= 200 && xhr.status < 300 || xhr.status == 304) {
        ...
        cb({
          "status": xhr.status,
          "msg": Ks3.xmlToJson(xhr.responseXML)['Error']['Message']
        }, null);
      } else {
        console.log('status: ' + xhr.status);
        cb({
...
/* harmony default export */ __webpack_exports__["a"] = (Ks3);
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__("9dce")))
...
/* WEBPACK VAR INJECTION */(function(plupload) {plupload.addI18n({
  "N\/A": "N\/A",
  "tb": "tb",
  "gb": "gb",
  "mb": "mb",
...
/* harmony default export */ __webpack_exports__["a"] = ({
  ...
  props: {
    ...
    mime_types: {
      ...
      default: function _default() {
        return [{
          title: "files ",
          extensions: "mp4,mov,qt,ts,rmvb,rm,avi,flv,mkv,wmv,mpg,mpeg,m2v,m4v,3gp,3g2,webm,vob,ogv,ogg,jpg,jpeg"
        }];
      }
    },
    //限制上传大小
  ...
  mounted: function mounted() {
    ...
    this.$nextTick(function () {
      if (!_this.Signature.bucket) throw new Error("请配置bucket!");
  ...
  methods: {
    remove: function remove(file) {
      // 在队列中移除文件
      this.uploader.uploader.removeFile(file); // 在ui上移除

      var index = this.fileList.findIndex(function (ele) {
    ...
    fileSize: function fileSize(_fileSize) {
      if (_fileSize < 1024) {
        ...
        var temp = _fileSize / 1024;
        temp = temp.toFixed(2);
        return temp + "KB";
      } else if (_fileSize < 1024 * 1024 * 1024) {
        var temp = _fileSize / (1024 * 1024);
        temp = temp.toFixed(2);
        return temp + "MB";
      } else {
        var temp = _fileSize / (1024 * 1024 * 1024);
        temp = temp.toFixed(2);
        return temp + "GB";
      }
    },
    Cutsubstr: function Cutsubstr(str, len) {
      if (!str || !len) {
        return "";
      }
    ...
    ks3UploadUrl: function ks3UploadUrl() {
      var protocol;
    ...
    upload: function upload() {
      ...
      var ks3Options = {
        ...
        onUploadProgressCallBack: function onUploadProgressCallBack(uploader, obj) {
          $(".file-".concat(obj.id, " .progress")).css("width", obj.percent + "%");
          $(".file-".concat(obj.id, " .file-status")).html(obj.percent + "%");
        },
        ...
        onErrorCallBack: function onErrorCallBack(uploader, errObject) {
          ...
          $(".file-".concat(errObject.file.id, " .progress")).css("background", "#ff9900");
          $(".file-".concat(errObject.file.id, " .file-status")).html("上传失败");
...
// CONCATENATED MODULE: ./src/components/ks3_upload/ks3_upload.vue?vue&type=template&id=6d597c9b&
...
// CONCATENATED MODULE: ./src/components/ks3_upload/ks3_upload.vue?vue&type=script&lang=js&
 /* harmony default export */ var ks3_upload_ks3_uploadvue_type_script_lang_js_ = (ks3_uploadvue_type_script_lang_js_["a" /* default */]); 
...
// CONCATENATED MODULE: ./src/components/ks3_upload/ks3_upload.vue
...
var component = normalizeComponent(
  ks3_upload_ks3_uploadvue_type_script_lang_js_,
  render,
  staticRenderFns,
  false,
  null,
...
/* harmony default export */ var ks3_upload = (component.exports);
// CONCATENATED MODULE: ./src/components/ks3_upload/index.js



var ks3_upload_install = function install(Vue) {
  if (install.installed) {
    return;
  }

  install.installed = true;
  Vue.component(ks3_upload.name, ks3_upload);
};

ks3_upload.install = ks3_upload_install;

if (typeof window !== 'undefined' && window.Vue) {
  window.Vue.use(ks3_upload);
}

/* harmony default export */ var components_ks3_upload = (ks3_upload);
// CONCATENATED MODULE: ./node_modules/@vue/cli-service/lib/commands/build/entry-lib.js


/* harmony default export */ var entry_lib = __webpack_exports__["default"] = (components_ks3_upload);
